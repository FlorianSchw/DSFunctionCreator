...
CodeCheck.Datasources <- function(path_to_dsPackages = NULL, package_type = NULL){
  ...
  Datasources_Results <- internal_datasources(codelines = Client_Codelines, Client_Analysis= Client_Analysis)


  options(DT.options = list(pageLength = 200))
  Datasources_Return <- datatable(Datasources_Results) %>%
    formatStyle(c("NullCheck", "ClassCheck"),
                backgroundColor = styleEqual(c("No Argument","Complete", "Incomplete", "Absent"), c("blue","green", "yellow", "red"))) %>%
    formatStyle("DatasourcesPresent",
