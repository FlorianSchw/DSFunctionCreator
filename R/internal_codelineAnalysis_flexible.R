...
internal_codelineAnalysis_flexible <- function(codelines = NULL, Client_Analysis = NULL){
  ...
  codecheck_arg_null_2a_flexible <- readLines(paste0(codecheckDirectory, "/codecheck_final_arg_null_2a_flexible.R"))
  codecheck_arg_null_2b_flexible <- readLines(paste0(codecheckDirectory, "/codecheck_final_arg_null_2b_flexible.R"))
    ...
    for (n in 1:length(Client_Analysis$Function_FileName)){
      ...
      for (i in 1:length(codelines[[n]][[1]])){
        ...
        if((grepl(codecheck_arg_null_1, codelines[[n]][[1]][i], fixed = TRUE)) &&
           (grepl(codecheck_arg_null_2a_flexible, codelines[[n]][[1]][i+1], fixed = TRUE)) &&
           (grepl(codecheck_arg_null_2b_flexible, codelines[[n]][[1]][i+1], fixed = TRUE)) &&
